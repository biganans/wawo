...
namespace wawo { namespace log {
	...
	void SysLogger::Syslog(int const& level, char const* const log, u32_t const& length ) {

//		printf("syslog: %s\n", log.c_str() );
#if WAWO_SPLIT_SYSLOG
		if( length > WAWO_SYSLOG_SPLIT_LENGTH ) {
			...
			do {
				...
				int left_len = ( length-_idx*WAWO_SYSLOG_SPLIT_LENGTH );

				if( left_len <= 0 ) { break;}

				int to_c_len = (left_len >= WAWO_SYSLOG_SPLIT_LENGTH)? WAWO_SYSLOG_SPLIT_LENGTH : left_len;
				strncpy(split,log + (_idx*WAWO_SYSLOG_SPLIT_LENGTH), to_c_len );
				...
				_try = (to_c_len == WAWO_SYSLOG_SPLIT_LENGTH) ;
			} while( _try ) ;

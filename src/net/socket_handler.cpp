...
#define INT_HANDLER_CONTEXT_T_TO_H_PACKET_1(NAME,HANDLER_FLAG,HANDLER_CLASS_NAME) \
	int socket_handler_context::invoke_##NAME##(WWSP<packet> const& p) { \
		if (m_flag&HANDLER_FLAG) { \
			WWRP<HANDLER_CLASS_NAME> _h = wawo::dynamic_pointer_cast<HANDLER_CLASS_NAME>(m_h); \
			return _h->##NAME##(WWRP<socket_handler_context>(this), p); \
		} else { \
			return NAME##(p); \
		} \
	} \
	 \
	int socket_handler_context::##NAME##(WWSP<packet> const& p) { \
		WAWO_ASSERT(P != NULL); \
		return P->invoke_##NAME##(p); \
	}

	INT_HANDLER_CONTEXT_T_TO_H_PACKET_1(write, socket_handler_context::F_OUTBOUND, socket_outbound_handler_abstract)

#define INT_HANDLER_CONTEXT_T_TO_H_INT_1(NAME,HANDLER_FLAG,HANDLER_CLASS_NAME) \
	int socket_handler_context::invoke_##NAME##(int const& i) { \
		if (m_flag&HANDLER_FLAG) { \
			WWRP<HANDLER_CLASS_NAME> _h = wawo::dynamic_pointer_cast<HANDLER_CLASS_NAME>(m_h); \
			return _h->##NAME##(WWRP<socket_handler_context>(this), i); \
		} else { \
			return NAME##(i); \
		} \
	} \
	 \
	int socket_handler_context::##NAME##(int const& i) { \
		WAWO_ASSERT(P != NULL); \
		return P->invoke_##NAME##(i); \
	}

	INT_HANDLER_CONTEXT_T_TO_H_INT_1(close, socket_handler_context::F_OUTBOUND, socket_outbound_handler_abstract)
	INT_HANDLER_CONTEXT_T_TO_H_INT_1(close_read, socket_handler_context::F_OUTBOUND, socket_outbound_handler_abstract)
	INT_HANDLER_CONTEXT_T_TO_H_INT_1(close_write, socket_handler_context::F_OUTBOUND, socket_outbound_handler_abstract)
...
#define INT_HANDLER_IMPL_INT_1(NAME,HANDLER_NAME) \
	int HANDLER_NAME##::##NAME##(WWRP<socket_handler_context> const& ctx, int const& code) { \
		return ctx->##NAME##(code); \
	}

	INT_HANDLER_IMPL_INT_1(close, socket_outbound_handler_abstract)
	INT_HANDLER_IMPL_INT_1(close_read, socket_outbound_handler_abstract)
	INT_HANDLER_IMPL_INT_1(close_write, socket_outbound_handler_abstract)
	...
	int socket_handler_head::write(WWRP<socket_handler_context> const& ctx, WWSP<packet> const& outlet)
	{
		return ctx->so->send_packet(outlet);
	}

	int socket_handler_head::close(WWRP<socket_handler_context> const& ctx, int const& code) {
		return ctx->so->close(code);
	}

	int socket_handler_head::close_read(WWRP<socket_handler_context> const& ctx, int const& code) {
		return ctx->so->shutdown(SHUTDOWN_RD, code);
	}

	int socket_handler_head::close_write(WWRP<socket_handler_context> const& ctx, int const& code) {
		return ctx->so->shutdown(SHUTDOWN_WR,code);
	}
	...
	int socket_handler_tail::write(WWRP<socket_handler_context> const& ctx, WWSP<packet> const& outlet)
	{

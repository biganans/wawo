...
namespace wawo { namespace net {
	...
	public:
		explicit socket(SOCKET const& fd, socket_mode const& sm, address const& laddr, address const& raddr,s_family const& family , s_type const& sockt, s_protocol const& proto):
			...
			channel(wawo::net::io_event_loop_group::instance()->next(determin_type_by_protocol(proto))),
			m_flag(0),
			m_state(S_CONNECTED),
			m_trb(NULL),
			m_noutbound_bytes(0)
		...
		static WWRP<channel_future> dial(std::string const& dialurl, fn_channel_initializer const& initializer, socket_cfg const& cfg = default_socket_cfg) {
			WWRP<wawo::net::channel_promise> f = wawo::make_ref<channel_promise>(nullptr);
			socket::dial(dialurl, initializer, f, cfg);
			return f;
		}

		static void dial(std::string const& dialurl, fn_channel_initializer const& initializer, WWRP<wawo::net::channel_promise> const& ch_promise, socket_cfg const& cfg = default_socket_cfg ) {
			socketaddr soaddr;
			...
			ch_promise->attach_channel(so);
			ch_promise->add_listener([](WWRP<channel_future> const& f) {
				if (f->get() != wawo::OK) {
					f->channel()->ch_close();
				}
			});
			so->dial(soaddr.so_address, initializer, ch_promise, cfg);
		}
		...
		*/
		static WWRP<channel_future> listen_on(std::string const& addrurl, fn_channel_initializer const& accepted, socket_cfg const& cfg = default_socket_cfg, socket_cfg const& child_cfg = default_socket_cfg, int backlog = WAWO_DEFAULT_LISTEN_BACKLOG) {
			...
			WWRP<socket> so = wawo::make_ref<socket>(soaddr.so_address.family(), soaddr.so_type, soaddr.so_proto);
			WWRP<channel_future> listen_future = so->listen_on(soaddr.so_address, accepted, cfg, child_cfg, backlog);
	...
	private:
		...
		WWRP<channel_future> listen_on(address const& addr, fn_channel_initializer const& fn_accepted, socket_cfg const& cfg, socket_cfg const&child_cfg = default_socket_cfg, int const& backlog = WAWO_DEFAULT_LISTEN_BACKLOG);
		void listen_on(address const& addr, fn_channel_initializer const& fn_accepted, WWRP<channel_promise> const& ch_promise, socket_cfg const& cfg, socket_cfg const& child_cfg = default_socket_cfg, int const& backlog = WAWO_DEFAULT_LISTEN_BACKLOG);

		WWRP<channel_future> dial(address const& addr, fn_channel_initializer const& initializer, socket_cfg const& cfg = default_socket_cfg);
		void dial(address const& addr, fn_channel_initializer const& initializer, WWRP<channel_promise> const& ch_promise, socket_cfg const& cfg = default_socket_cfg);

		inline void _ch_do_shutdown_read( WWRP<channel_promise> const& f ) {
			WAWO_ASSERT((m_flag&F_READ_SHUTDOWN) == 0);
			socket::ch_async_io_end_read();
		...
		inline void __cb_async_connect_impl(async_io_result const& r) {
			...
			if (code == wawo::OK && local_addr() == remote_addr()) {
				WAWO_ERR("[#%d]new fd self connected", fd());
		...
		inline void __async_flush_done(int code) {
			switch (code) {
			case wawo::OK:
			{
				//check send result
		...
		inline void __begin_write_impl(fn_io_event const& fn_write = NULL) {
			...
			if (!event_poller()->in_event_loop()) {
				...
				event_poller()->execute([_so, fn_write]()->void {
					_so->__begin_write_impl(fn_write);
				});
				return;
			}
		...
		inline void __end_write_impl() {
			if (!event_poller()->in_event_loop()) {
				WWRP<socket> _so(this);
				event_poller()->execute([_so]()->void {
					_so->__end_write_impl();
				});
				return;
			}
			WAWO_ASSERT(is_nonblocking());
	...
	private:
		__WW_FORCE_INLINE void _async_io_begin_connect() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
			}
#endif
			__begin_write_impl(std::bind(&socket::__cb_async_connect_impl, WWRP<socket>(this), std::placeholders::_1));
		}
		__WW_FORCE_INLINE void async_io_end_connect() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
		...
		__WW_FORCE_INLINE void _async_io_begin_accept() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
...
#endif
			ch_async_io_begin_read(F_WATCH_READ_INFINITE,std::bind(&socket::__cb_async_accept_impl, WWRP<socket>(this), std::placeholders::_1));
		}
		__WW_FORCE_INLINE void async_io_end_accept() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
		...
		__WW_FORCE_INLINE void _async_io_begin_write() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
		...
		__WW_FORCE_INLINE void _async_io_end_write() {
#ifdef WAWO_IO_MODE_IOCP
			if (event_poller()->poller_type() == T_IOCP) {
				WAWO_ASSERT(!"TODO");
				return;
		...
		void __IOCP_cb_async_accept(async_io_result const& r) {
			WAWO_ASSERT(m_fn_accept_initializer != NULL);
			int ec = r.v.code;
			...
			if (ec == wawo::OK) {
				...
				if (raddr_in == laddr_in) {
					WAWO_ERR("[socket][%s][accept]raddr == laddr, close: %u", info().to_stdstring().c_str(), nfd);
					WAWO_CLOSE_SOCKET(nfd);
				}
				else {
		...
		inline int __IOCP_CALL_IMPL_ConnectEx(void* ol_) {
			...
			if (connrt == FALSE) {
				...
				if (ec != wawo::E_ERROR_IO_PENDING) {
					WAWO_DEBUG("[socket][iocp][#%u]socket __connectEx failed", fd(), connrt);
					return ec;
				}
			}
			WAWO_DEBUG("[socket][iocp][#%u]connectex ok", fd());
			return wawo::OK;
		}

		void __IOCP_cb_async_read(async_io_result const& r) {
			(void)r;
			WAWO_ASSERT(event_poller()->in_event_loop());
			WAWO_ASSERT(!is_listener());
			WAWO_ASSERT(is_nonblocking());
		...
		//non null, send
		//null clear
		inline int __IOCP_INIT_IMPL_WSASend( void* ol_ ) {
			WSAOVERLAPPED* ol = (WSAOVERLAPPED*)ol_;
		...
		inline void __IOCP_CALL_CB_WSASend(async_io_result const& r) {
			...
			if ( WAWO_LIKELY(len>0) ) {
				...
				socket::__async_flush_done(wawo::OK);
			} else {
				socket::__async_flush_done(r.v.code);
			}
		}

		inline void __IOCP_INIT_WSASend() {
			event_poller()->IOCP_overlapped_call(IOE_WRITE, fd(), std::bind(&socket::__IOCP_INIT_IMPL_WSASend, WWRP<socket>(this), std::placeholders::_1),
				std::bind(&socket::__IOCP_CALL_CB_WSASend, WWRP<socket>(this), std::placeholders::_1));
		}
		inline void __IOCP_begin_write(fn_io_event const& fn_write = NULL) {
			(void)fn_write;
			int flushrt = _ch_ch_flush_impl();
			if (flushrt != wawo::OK) {
				ch_errno(flushrt);
				ch_close();
			}
		}
		inline void __IOCP_end_write() {
			WAWO_ASSERT(!"TODO");
		}
		inline int __IOCP_do_ch_flush_impl() {
			WAWO_ASSERT(event_poller()->in_event_loop());
...
#endif
	public:
		inline void ch_async_io_begin_read(u8_t const& async_flag = F_WATCH_READ_INFINITE, fn_io_event const& fn_read = NULL) {
			WAWO_ASSERT(is_nonblocking());
			if (!event_poller()->in_event_loop()) {
				WWRP<socket> _so(this);
				event_poller()->execute([_so,async_flag,fn_read]()->void {
					_so->ch_async_io_begin_read(async_flag,fn_read);
				});
				return;
			}

			if (m_flag&F_WATCH_READ) {
				WAWO_TRACE_IOE("[socket][%s][begin_read]ignore for watch already", info().to_stdstring().c_str());
				return;
			}

			if (m_flag&F_READ_SHUTDOWN) {
				WAWO_TRACE_IOE("[socket][%s][begin_read]cancel for rd shutdowned already", info().to_stdstring().c_str());
				if (fn_read != NULL) {
					fn_read({ AIO_READ, m_fd, wawo::E_CHANNEL_READ_SHUTDOWN_ALREADY , 0 });
				}
				return;
			}
			const fn_io_event _fn_io = fn_read == NULL ? std::bind(&socket::__cb_async_read_impl, WWRP<socket>(this),std::placeholders::_1) : fn_read ;
		...
		inline void ch_async_io_end_read() {
			if (!event_poller()->in_event_loop()) {
				WWRP<socket> _so(this);
				event_poller()->execute([_so]()->void {
					_so->ch_async_io_end_read();
				});
				return;
			}
			WAWO_ASSERT(is_nonblocking());
			if ((m_flag&F_WATCH_READ) ) {
				...
				WAWO_TRACE_IOE("[socket][%s][end_read]unwatch IOE_READ", info().to_stdstring().c_str());
		...
		void ch_shutdown_read_impl(WWRP<channel_promise> const& ch_promise)
		{
			...
			_ch_do_shutdown_read(ch_promise);
			__rdwr_shutdown_check();
		}

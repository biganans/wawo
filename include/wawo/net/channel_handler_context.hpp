...
namespace wawo { namespace net {
	...
	public:
		...
		void begin_read(u8_t const& async_flag , WWRP<ref_base> const& cookie, fn_io_event const& fn, fn_io_event_error const& err);
		void end_read();

		void begin_connect(WWRP<ref_base> const& cookie, fn_io_event const& fn, fn_io_event_error const& err);
		void end_connect();

		void begin_write(u8_t const& async_flag, WWRP<ref_base> const& cookie, fn_io_event const& fn , fn_io_event_error const& err );
		void end_write();
	};

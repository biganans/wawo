...
#include <wawo/thread/impl/spin_mutex.hpp>
#include <wawo/thread/impl/mutex.hpp>
#include <wawo/thread/impl/condition.hpp>
	...
	namespace impl
	{
		...
		public:
			...
			inline void sleep_for(std::chrono::duration<_Rep, _Period> const& duration) {
				wawo::thread::unique_lock<spin_mutex> ulk(mtx);
				if (interrupted) {
					interrupted = false;
			...
			inline void check_interrupt() {
				wawo::thread::lock_guard<spin_mutex> lg(mtx);
				if (interrupted) {
					interrupted = false;
...
#ifdef _DEBUG_THREAD_CREATE_JOIN
			wawo::thread::thread_debugger::instance()->increase_create();
#endif
			return wawo::OK;
		}
#endif
...
#ifdef _DEBUG_THREAD_CREATE_JOIN
			wawo::thread::thread_debugger::instance()->increase_join();
#endif
		}

...
namespace wawo { namespace algorithm {
	...
	namespace bigendian_bytes_helper {
		//big endian 0x01020304 => 01 02 03 04 (in memory, from left -> right, addr incre)
		...
		template <class InIt>
		inline u32_t read_bytes( byte_t* const target, u32_t const& length,InIt const& start ) {
		...
		template <class OutIt>
		inline wawo::u32_t write_bytes( const byte_t* const buffer, wawo::u32_t const& length, OutIt const& out ) {
			wawo::u32_t write_idx = 0;

...
namespace wawo { namespace signal {
	...
	int SignalManager::RegisterSignal(int signo , SignalHandler_Abstract* signalHandler) {
...
#if WAWO_PLATFORM_WIN
		if( signo == SIGINT ) {
			WAWO_LOG_WARN( "SignalManager", "WIN32 do not support SIGINT, USE CTRL_C_EVENT, or CTRL_BREAK_EVENT" );
		}
#endif
		...
		if( it == m_signalHandlers.end() ) {
			...
			WAWO_LOG_WARN( "SignalManager", "register signo handler: %d", signo );

		} else {
			WAWO_LOG_WARN( "SignalManager", "register signo handler: %d, add", signo );
	...
	int SignalManager::_RegisterCtrlEventHandler() {
		...
		if( SetConsoleCtrlHandler( (PHANDLER_ROUTINE) SignalManager::CtrlEvent_Handler , TRUE ) == FALSE ) {
			WAWO_LOG_CORE( "SignalManager", "SetConsoleCtrlHandler failed, errno: %d", GetLastError() );
			return -1;
		}
	...
	BOOL WINAPI SignalManager::CtrlEvent_Handler( DWORD CtrlType ) {
		...
		switch( CtrlType ) {
			...
			case CTRL_C_EVENT:
				{
					raise( SIGINT );
					WAWO_LOG_WARN( "SignalManager", "CTRL_C_EVENT received" );
				}
				break;
			case CTRL_BREAK_EVENT:
				{
					raise( SIGINT );
					WAWO_LOG_WARN( "SignalManager", "CTRL_BREAK_EVENT received" );
				}
				break;
			...
			case CTRL_SHUTDOWN_EVENT:
				{
					raise( SIGTERM );
					WAWO_LOG_WARN( "SignalManager", "CTRL_SHUTDOWN_EVENT received" );
				}
				break;
			case CTRL_CLOSE_EVENT:
				{
					raise( SIGTERM );
					WAWO_LOG_WARN( "SignalManager", "CTRL_CLOSE_EVENT received" );
				}
				break;
			...
			case CTRL_LOGOFF_EVENT:
				{
					raise( SIGTERM );
   					WAWO_LOG_WARN( "SignalManager", "CTRL_LOGOFF_EVENT received" );
				}
				break;
	...
	void SignalManager::_SignalDispatch( int signo ) {
		...
		if( it == m_signalHandlers.end() ) {
			...
			WAWO_LOG_WARN( "SignalManager", "no hander for signo: %d", signo );
			return ;
		}

#ifndef _WAWO_LOG_LOGGER_H_
#define _WAWO_LOG_LOGGER_H_
...
namespace wawo { namespace log {
	...
	public:
		...
		virtual void Write( LogLevelMask const& level, char const* const log, uint32_t const& length ) = 0 ;
	
		virtual void Debug( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_DEBUG, log, length );
		}
		virtual void Info( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_INFO, log, length );
		}
		virtual void Notice( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_NOTICE, log, length );
		}
		virtual void Warn( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_WARN, log, length );
		}
		virtual void Fatal( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_FATAL, log, length );
		}
		virtual void Core( char const* const log, uint32_t const& length ) {
			Write( Logger_Abstract::LOG_LEVEL_CORE, log, length );
		}
		inline void SetFormat( FormatInterface* format ){
			m_format = format ;
		}
		inline FormatInterface* const& GetFormat() {
			return m_format;
		}

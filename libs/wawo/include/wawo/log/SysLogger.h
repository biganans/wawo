...
#include <wawo/log/Logger_Abstract.hpp>
	...
	namespace wawo { namespace log {
		...
		public:
			 ...
			 */
			void Write( LogLevelMask const& level, char const* const log, uint32_t const& length ) = 0 ;

		private:
			static void Syslog( int const& level, char const* const log, uint32_t const& length );
			char m_ident[32] ;
		};
	}}
